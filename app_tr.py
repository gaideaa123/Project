...
DEFAULT_GUIDE = "Doğal Türkiye Türkçesiyle iki kısa cümle ve ikinci satırda tam 5 alakalı hashtag yaz."
...
class AzureTitleClient:
    def __init__(self, key: str, url: str, guide: str):
        self.key, self.url, self.guide = key.strip(), url.strip(), guide.strip() or DEFAULT_GUIDE
    ...
    def create(self, profile: str) -> str:
        ...
        for attempt in range(3):
            try:
                ...
                if not response.ok:
                    error = payload.get("error", {}); raise RuntimeError(error.get("message") if isinstance(error, dict) else str(error))
...
class PublishWorker(QThread):
    ...
    def __init__(self, profiles, source, key, url, guide, parent=None):
    ...
    def run(self):
        completed = 0
        try:
            self.status.emit("Web yükleyici için 1.mp4 - 5.mp4 oluşturuluyor")
            variants = video_variants.create_five_variants(
                self.source, lambda percent, text: (self.progress.emit(percent), self.status.emit(text))
            )
            self.variants_ready.emit([str(path) for path in variants])
            client = AzureTitleClient(self.key, self.url, self.guide)
            for index, profile in enumerate(self.profiles):
                if self._cancelled: break
                video = variants[index % 5]
...
class TurkceAnaPencere(core.MainWindow):
    ...
    def azure_web_tab(self):
        ...
        note = QLabel("Kaynak video otomatik 1.mp4, 2.mp4, 3.mp4, 4.mp4, 5.mp4 olarak işlenir. Seçili profillere sırayla dağıtılır; Azure caption yazıp doğrular.")
        ...
        settings.addRow("Azure API Key", self.azure_key); settings.addRow("Azure URL", self.azure_url); settings.addRow("Caption kuralları", self.azure_guide)
        ...
        self.variant_progress = QProgressBar(); self.variant_progress.setRange(0, 100); layout.addWidget(self.variant_progress)
        ...
        publish = QPushButton("5 VARYASYON OLUŞTUR + WEB İLE YAYINLA"); publish.clicked.connect(self.publish_selected)
        ...
        for button in (save, select_all, publish, self.cancel_publish): controls.addWidget(button)
    ...
    def edit_tiktok_session(self, profile):
        ...
        reveal = QCheckBox("Göster"); reveal.toggled.connect(lambda checked: field.setEchoMode(QLineEdit.Normal if checked else QLineEdit.Password)); layout.addWidget(reveal)
    ...
    def save_azure(self, show_message=True):
        try:
            client = AzureTitleClient(self.azure_key.text(), self.azure_url.text(), self.azure_guide.text())
            del client
            for key, value in {"api_key":self.azure_key.text().strip(), "api_url":self.azure_url.text().strip(), "guide":self.azure_guide.text().strip()}.items(): keyring.set_password(AZURE_SERVICE, key, value)
    ...
    def publish_profiles(self, profiles):
        try:
            ...
            worker = PublishWorker(profiles, source, self.azure_key.text(), self.azure_url.text(), self.azure_guide.text(), self)
    ...
    def show_variants(self, paths):
        self.variant_folder.setText(f"Varyasyon klasörü: {Path(paths[0]).parent} | Dosyalar: 1.mp4, 2.mp4, 3.mp4, 4.mp4, 5.mp4")
