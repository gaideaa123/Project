...
Each browser profile inherits its tested proxy assignment from Proxy Listesi.
The module does not bypass login, CAPTCHA, 2FA, platform review, or final user
confirmation.
"""
...
def save_diagnostics(page: Page, profile: str, error: Exception) -> Path:
 ...
 (folder / "error.json").write_text(
  json.dumps(
   {"profile": profile, "url": page.url, "error": str(error), "time": datetime.now().isoformat()},
   ensure_ascii=False,
   indent=2,
  ),
  encoding="utf-8",
...
def assigned_proxy(profile: str) -> dict[str, str] | None:
 """Return only a saved, currently healthy proxy for this exact profile."""
 ...
 try:
  ...
  raise UploadError(f"{profile}: atanmış proxy sağlık kontrolünü geçmedi: {exc}") from exc
