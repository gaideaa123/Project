...
"""Visible TikTok Studio uploader with target-specific proxy preflight."""
...
from playwright.sync_api import BrowserContext, Locator, Page, Playwright, TimeoutError as PlaywrightTimeout, sync_playwright
...
class UploadRequest:
 ...
 def validate(self) -> None:
  ...
  if not self.video.is_file() or self.video.suffix.lower() not in MEDIA_EXTENSIONS: raise UploadError(f"Geçerli video bulunamadı: {self.video}")
...
def wait_for_login(page: Page, timeout_seconds: int = 600) -> None:
 ...
 while time.monotonic() < deadline:
  if "/login" not in page.url.lower() and locator_attached(page.locator('input[type="file"]')): return
...
def upload_file(page: Page, video: Path) -> None:
 ...
 try:
  if locator_attached(direct): direct.first.set_input_files(str(video.resolve())); return
 ...
 if trigger is None: raise UploadError("Video seçim alanı bulunamadı. TikTok Studio arayüzü değişmiş olabilir")
 try:
  with page.expect_file_chooser(timeout=5000) as chooser_info: trigger.click()
...
def caption_candidates(page: Page) -> list[Locator]:
 return [
  ...
  page.locator('textarea[placeholder*="caption" i]'), page.locator('textarea[placeholder*="description" i]'),
...
def fill_caption(page: Page, caption: str, timeout_seconds: int = 180) -> None:
 ...
 while time.monotonic() < deadline:
  ...
  if field is not None:
   field.click(); tag = field.evaluate("el => el.tagName.toLowerCase()")
   if tag in {"textarea", "input"}: field.fill(caption)
   else: page.keyboard.press("Control+A"); page.keyboard.insert_text(caption)
   return
  page.wait_for_timeout(1000)
...
def wait_until_ready(page: Page, timeout_seconds: int = 600) -> None:
 ...
 while time.monotonic() < deadline:
  try:
   if failure.search(page.locator("body").inner_text(timeout=2000)): raise UploadError("TikTok videoyu reddetti: sayfadaki hata mesajını kontrol edin")
  except PlaywrightTimeout: pass
  publish = first_visible([page.get_by_role("button", name=re.compile(r"^post$|^publish$|^yayınla$|^paylaş$", re.I)), page.locator('button[data-e2e*="post" i]')], 500)
  if publish is not None:
   try:
    if publish.is_enabled(): return
   except Exception: pass
...
def save_diagnostics(page: Page, profile: str, error: Exception) -> Path:
 folder = DATA_ROOT / "diagnostics" / datetime.now().strftime("%Y%m%d-%H%M%S-%f"); folder.mkdir(parents=True, exist_ok=True)
 try: page.screenshot(path=str(folder / "page.png"), full_page=True)
 except Exception: pass
 try: (folder / "page.html").write_text(page.content(), encoding="utf-8")
 except Exception: pass
 (folder / "error.json").write_text(json.dumps({"profile": profile, "url": page.url, "error": str(error), "time": datetime.now().isoformat()}, ensure_ascii=False, indent=2), encoding="utf-8")
 return folder
...
def launch_context(playwright: Playwright, profile: str) -> BrowserContext:
 ...
 for candidate in browser_proxy_candidates(identity):
  context = None
  try:
   options = dict(base); options["proxy"] = candidate
   context = _launch(playwright, options)
   proxy_health.verify_browser_target(context, identity, UPLOAD_URL)
   return context
  except Exception as exc:
...
def goto_upload(page: Page) -> None:
...
def prepare_upload(request: UploadRequest) -> None:
 ...
 with sync_playwright() as playwright:
  ...
  try:
   goto_upload(page); wait_for_login(page); upload_file(page, request.video); fill_caption(page, request.caption); wait_until_ready(page)
   page.bring_to_front(); print("HAZIR: Video ve caption dolduruldu. TikTok ayarlarını kontrol edip Yayınla düğmesine siz basın.")
...
def main() -> int:
 ...
 parser.add_argument("--profile", required=True); parser.add_argument("--video", required=True, type=Path)
 parser.add_argument("--caption"); parser.add_argument("--caption-file", type=Path); args = parser.parse_args()
 caption = args.caption or (args.caption_file.read_text(encoding="utf-8") if args.caption_file else "")
 try: prepare_upload(UploadRequest(args.profile, args.video.expanduser().resolve(), caption)); return 0
