...
"""Session bootstrap and conservative TikTok web-publishing integration."""
...
def bootstrap_session(context, profile: str) -> bool:
    """Bootstrap only an empty persistent profile; never overwrite continuity."""
...
def wait_for_upload_after_login(page, timeout_seconds=900, status=None, profile=""):
    ...
    while time.monotonic() < deadline:
        ...
        if "/login" in url or _visible(page.locator('input[type="password"]')):
            if not reported and status:
                status(
                    f"{profile}: Session ID reddedildi veya ek doğrulama gerekiyor; "
                    "Chrome içinde doğrulamayı tamamlayın"
                )
            reported = True
            page.bring_to_front()
...
def install(web_uploader: Any):
    with _INSTALL_LOCK:
        ...
        original_ready = web_uploader.wait_for_upload_complete

        def launch(playwright, profile):
            context = original_launch(playwright, profile)
            bootstrap_session(context, profile)
            return context

        def confirm(page):
            if copyright_dialog.handle(page):
                return
        ...
        def ready(page, status=None, timeout_seconds=900):
        ...
        def prepare(request, publish=False, approval=None, status=None):
            with _INSTALL_LOCK:
                old_wait = web_uploader.wait_for_upload_after_login
                old_confirm = web_uploader.confirm_publish_dialog
                old_writer = web_uploader._write_caption
                old_result = web_uploader.wait_for_publish_result
                ...
                def verified(page, timeout_seconds=180):
                    return publication_guard.wait_for_verified_publication(
                        page, request.profile, status, timeout_seconds
                    )

                web_uploader.wait_for_upload_after_login = waiter
                web_uploader.confirm_publish_dialog = confirm
                web_uploader._write_caption = _native_caption_write
                web_uploader.wait_for_publish_result = verified
                if status:
                    status(
                        f"{request.profile}: Session ID sürekliliği ve sıfır erişim risk kapıları etkin"
                    )
                try:
                    ...
                    web_uploader.wait_for_upload_after_login = old_wait
                    web_uploader.confirm_publish_dialog = old_confirm
                    web_uploader._write_caption = old_writer
                    web_uploader.wait_for_publish_result = old_result

        web_uploader.launch_context = launch
        web_uploader.confirm_publish_dialog = confirm
        web_uploader.dismiss_pre_caption_notice = notice
        web_uploader.wait_for_upload_complete = ready
        web_uploader._write_caption = _native_caption_write
