...
"""Proxy connectivity verification with best-effort metadata enrichment."""
...
def fingerprint(identity: network_identity.NetworkIdentity) -> str:
    return hashlib.sha256(
        f"{identity.server}|{identity.username}".encode()
    ).hexdigest()
...
def _fetch_ip(session: requests.Session, timeout: int) -> str:
    ...
    for url, field in IP_ENDPOINTS:
        try:
            ...
            value = _extract_ip(response.json(), field)
            if value:
                return value
...
def _direct_ip() -> str:
    try:
        with _session() as session:
            return _fetch_ip(session, 8)
    except Exception:
        return ""


def _geo_metadata(
    session: requests.Session, exit_ip: str, timeout: int
) -> tuple[str, str, str, str]:
    """Return country, ASN, organisation and warning. Never gates connectivity."""
    errors: list[str] = []
    for template in GEO_ENDPOINTS:
        ...
        try:
            ...
            if data.get("error") is True or data.get("success") is False:
                raise ValueError(str(data.get("reason") or data.get("message") or "geo error"))
            country = str(
                data.get("country_code") or data.get("country_code2") or data.get("country_code_iso2") or ""
            ).upper()
...
def test(
    identity: network_identity.NetworkIdentity, attempts: int = 3, timeout: int = 15
) -> ProxyHealth:
    identity.validate()
    exits: list[str] = []
    latencies: list[int] = []
    ...
    try:
        with _session(proxy_map) as session:
            for _ in range(max(2, attempts)):
                ...
                exits.append(_fetch_ip(session, timeout))
                latencies.append(round((time.perf_counter() - started) * 1000))

            if not exits[0]:
                raise ProxyHealthError("Proxy çıkış IP döndürmedi")
            if len(set(exits)) != 1:
                raise ProxyHealthError(
                    "Proxy sabit çıkış IP vermedi: " + ", ".join(sorted(set(exits)))
                )
            if direct and exits[0] == direct:
                raise ProxyHealthError(
                    "Proxy devreye girmedi; çıkış IP doğrudan bağlantıyla aynı"
                )
            ...
            joined = f"{asn} {org}".casefold()
            flags: list[str] = []
            if any(
                marker in joined
                for marker in (
                    "hosting", "cloud", "datacenter", "digitalocean", "hetzner",
                    "ovh", "amazon", "google cloud", "microsoft",
                )
            ):
                flags.append("datacenter_or_hosting")

            detail = "HTTPS ve sabit çıkış IP doğrulandı"
            if geo_warning:
                detail += "; " + geo_warning
            result = ProxyHealth(
                fingerprint(identity), True, exits[0], country, median,
                datetime.now(UTC).isoformat(), detail, asn, org, tuple(flags),
            )
    except Exception as exc:
        result = ProxyHealth(
...
def verify_browser_context(context, identity: network_identity.NetworkIdentity) -> ProxyHealth:
    ...
    if not actual:
        raise ProxyHealthError(
            "Browser-context proxy testi başarısız: " + ", ".join(errors)
        )
    if actual != expected.exit_ip:
        raise ProxyHealthError(
            f"Browser-context çıkış IP değişti: beklenen {expected.exit_ip}, görülen {actual}"
        )
    return expected
