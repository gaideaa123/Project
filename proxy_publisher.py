 ...
 class ProxyAwareTikTokPublisher(core.TikTokPublisher):
  ...
  def token(self, account: dict[str, Any]) -> str:
   ...
   response = self._request(
    "POST", f"{core.TIKTOK_API}/v2/oauth/token/", account,
   ...
   self.registry.update_account(
    account["id"],
    token_expires_at=core.to_iso(core.now_utc() + timedelta(seconds=int(payload.get("expires_in", 3600)))),
   )
   return access
  ...
  def publish(self, account: dict[str, Any], job: dict[str, Any], signals: core.ThreadSignals) -> str:
   ...
   info = self._json(self._request(
    "POST", f"{core.TIKTOK_API}/v2/post/publish/creator_info/query/", account,
   ...
   initialized = self._json(self._request(
    "POST", f"{core.TIKTOK_API}/v2/post/publish/video/init/", account,
    headers=headers,
    json={
     "post_info": {"title": job["caption"][:2200], "privacy_level": privacy,
   ...
   with video.open("rb") as handle:
    for index in range(chunk_count):
     ...
     response = self._request(
      "PUT", upload_url, account, data=body,
...
def install_proxy_backend(window: Any) -> str:
 session_account_gui.install(window)
 if ProxyAwareTikTokPublisher is not None and all(hasattr(window, name) for name in ("registry", "secrets")):
  window.publisher = ProxyAwareTikTokPublisher(window.registry, window.secrets)
  return "publisher"
 if ProxyAwareTikTokClient is not None and all(hasattr(window, name) for name in ("registry", "vault")):
