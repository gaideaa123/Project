...
"""Local quality gates for genuinely edited short-form video outputs."""
...
def inspect_video(path: Path, samples: int = 16) -> OriginalityReport:
 ...
 if not capture.isOpened():
  raise OriginalityQAError(f"Video OpenCV ile açılamadı: {path.name}")
 try:
  frame_count = int(capture.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
  fps = float(capture.get(cv2.CAP_PROP_FPS) or 0)
  duration = frame_count / fps if frame_count > 0 and fps > 0 else 0.0
  if frame_count < 2 or duration <= 0:
   raise OriginalityQAError(f"Video süresi/frame sayısı okunamadı: {path.name}")
  ...
  for position in positions:
   ...
   try:
    ...
    pass
  static_ratio = static / transitions if transitions else 1.0
  return OriginalityReport(duration, len(signatures), static_ratio, qr_detected, tuple(signatures))
 finally:
  capture.release()
