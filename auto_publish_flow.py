...
"""Automatic publish only after completed checks and no eligibility advisory."""
...
HARD_FAILURE = re.compile(r"yükleme başarısız|video yüklenemedi|paylaşım başarısız|gönderilemedi|upload failed|failed to upload|publish failed|couldn.t post", re.I)
...
def wait_for_checks_complete(page, status: Status | None = None, timeout_seconds: int = 900):
 ...
 while time.monotonic() < deadline:
  ...
  if failed: raise RuntimeError(f"TikTok yükleme hatası gösteriyor: {failed.group(0)}")
 ...
 def prepare_upload(request, publish: bool = False, approval=None, status: Status | None = None):
  ...
  with sync_playwright() as playwright:
   ...
   try:
    ...
    web_uploader.goto_upload(page); web_uploader.wait_for_login(page); web_uploader.upload_file(page, request.video); web_uploader.fill_caption(page, request.caption)
