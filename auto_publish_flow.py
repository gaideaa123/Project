...
"""Automatic publish after completed checks, with safe repeated-dialog recovery."""
...
CHECK_OK = r"(?:sorun\s+tespit\s+edilmedi|kontrol(?:ler)?\s+tamamlandı|no\s+(?:issues?|problems?)\s+(?:detected|found)|(?:check\s+)?passed|complete(?:d)?)"
COPYRIGHT_COMPLETE = re.compile(
 rf"(?:müzik\s+telif\s+hakkı\s+kontrolü|telif\s+hakkı\s+kontrolü|copyright\s+check|music\s+copyright).{{0,160}}{CHECK_OK}",
 re.I | re.S,
)
CONTENT_COMPLETE = re.compile(
 rf"(?:içerik\s+kontrolü(?:\s*\([^)]*\))?|content\s+check).{{0,160}}{CHECK_OK}",
 re.I | re.S,
)
...
REPEATED_DIALOG_COOLDOWN_SECONDS = 12.0
...
def _all_check_rows_complete(text: str) -> bool:
 """Require both visible TikTok check rows after an incomplete-check dialog."""
 return bool(COPYRIGHT_COMPLETE.search(text) and CONTENT_COMPLETE.search(text))
...
def wait_for_checks_complete(
 ...
 while time.monotonic() < deadline:
  ...
  completed = _all_check_rows_complete(text) if require_completion_evidence else bool(CHECK_COMPLETE.search(text))
  ...
  if button is not None:
   try:
    ...
    enabled = False
  if enabled and not pending and not visible_failure and completed and now >= not_before:
   stable += 1
   if stable >= 3:
    _status(status, "Video yüklendi, içerik ve telif kontrolleri tamamlandı; Paylaş hazır")
    return button
  else:
   stable = 0
  if now - last_notice >= 8:
   message = "Tekrarlanan uyarı kapatıldı; içerik ve telif kontrol satırlarının ikisinin de tamamlanması bekleniyor" if require_completion_evidence else "Video yükleme ve içerik kontrolünün tamamlanması bekleniyor"
...
def _dialog_cancel_button(dialog):
 ...
 try:
  ...
  for index in range(min(candidates.count(), 20)):
   ...
   label = re.sub(r"\s+", " ", candidate.get_attribute("aria-label") or candidate.inner_text(timeout=400) or "").strip()
...
def dismiss_incomplete_check_dialog(page, status: Status | None = None, timeout_seconds: float = 8.0) -> bool:
   ...
   for index in range(count):
    ...
    try:
     ...
     if cancel is None or not _click(cancel, page):
      raise RuntimeError("TikTok eksik kontrol uyarısı açıldı fakat İptal/Kapat düğmesine tıklanamadı")
 ...
 def prepare_upload(request, publish=False, approval=None, status=None):
  ...
  with sync_playwright() as playwright:
   ...
   try:
    ...
    publish_deadline = time.monotonic() + 900
    warning_count = 0
    require_all_results = False
    not_before = 0.0
    while time.monotonic() < publish_deadline:
     remaining = max(1, int(publish_deadline - time.monotonic()))
     button = wait_for_checks_complete(
      page,
      status=status,
      timeout_seconds=remaining,
      require_completion_evidence=require_all_results,
     ...
     if not dismiss_incomplete_check_dialog(page, status=status):
      break
     warning_count += 1
     require_all_results = True
     not_before = time.monotonic() + REPEATED_DIALOG_COOLDOWN_SECONDS
     _status(status, f"{request.profile}: eksik kontrol uyarısı {warning_count}. kez kapatıldı; Hemen paylaş kullanılmadan kontroller bekleniyor")
    else:
     raise RuntimeError("TikTok içerik kontrolünü 15 dakikada tamamlamadı")
