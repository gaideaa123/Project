...
"""Automatic publish only after completed checks and verified upload state."""
...
CHECK_PENDING = re.compile(
 ...
 r"(?:content|copyright|music).{0,35}(?:checking|in progress|processing|pending|not finished)", re.I,
)
...
ADVISORY = re.compile(
 ...
 r"unoriginal, low-quality|qr code content|not eligible for recommendation", re.I,
)
...
def wait_for_checks_complete(page,status:Status|None=None,timeout_seconds:int=900):
 """Wait through upload/checks; fail only on a persistent visible error."""
 deadline=time.monotonic()+timeout_seconds;stable=0;last_notice=0.0
 ...
 while time.monotonic()<deadline:
  ...
  if advisory:
   raise RuntimeError("TikTok içeriği özgün olmayan/düşük kaliteli/QR kodlu olarak sınıflandırdı. 'Yine de paylaş' seçilmedi; videoyu yeniden üretin.")
  visible_failure=upload_state.visible_upload_failure(page)
  if visible_failure:
   key=visible_failure.text.casefold()
   failure_hits=failure_hits+1 if key==failure_key else 1;failure_key=key
   if failure_hits>=3:
    selected=upload_state.selected_file(page)
    detail=f" Seçili dosya: {selected[0]} ({selected[1]} bayt)." if selected else ""
    raise RuntimeError(f"TikTok görünür ve kalıcı yükleme hatası gösteriyor: {visible_failure.text}.{detail}")
  else:
   failure_key="";failure_hits=0
  ...
  if enabled and not pending and not visible_failure:
   stable+=1
   if stable>=3:
    _status(status,"Video yüklendi, içerik kontrolleri tamamlandı; Paylaş hazır");return button
  else:stable=0
  now=time.monotonic()
  if now-last_notice>=8:
   _status(status,"Video yükleme ve içerik kontrolünün tamamlanması bekleniyor");last_notice=now
 ...
 def prepare_upload(request,publish:bool=False,approval=None,status:Status|None=None):
  ...
  with sync_playwright() as playwright:
   ...
   try:
    ...
    web_uploader.goto_upload(page);web_uploader.wait_for_login(page);web_uploader.upload_file(page,request.video)
    selected=upload_state.selected_file(page)
    ...
    button=wait_for_checks_complete(page,status=status);page.bring_to_front()
    if not publish:
